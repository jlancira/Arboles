...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    p3 <- ggplot(house_prices, aes(x = bathrooms)) + 
      labs(x = "bathrooms", title = "Bathrooms")+
      geom_histogram(bins = bins, color = colorline, fill = colorfill, bandwidth =1)
    
    p4 <- ggplot(house_prices, aes(x = bathrooms, y = log10_price, color = condition)) +
